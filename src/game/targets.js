...
const targets_bodies = {
    ...
    battlemage: {name: "Battlemage",hp: 3, sp: 0, mp: 1, stats: {str: 1, dex: 1, int: 1}},
...
const targets_mods = {
    ...
    enduring: {name: "Enduring",   hp: -3, sp: 4, mp: -1, stats: {str: -1, dex: 2, int: -1}},
    brilliant: {name: "Brilliant", hp: -3, sp: -2, mp: 2, stats: {str: -1, dex: -1, int: 2}},
    athlete: {name: "Athlete",     hp: 3, sp: 2, mp: -2, stats: {str: 1, dex: 1, int: -2}},
    forceful: {name: "Forceful",   hp: 3, sp: -4, mp: 1, stats: {str: 1, dex: -2, int: 1}},
    sly: {name: "Sly",             hp: -6, sp: 2, mp: 1, stats: {str: -2, dex: 1, int: 1}},
    flat: {name: "Vulgaris",       hp: 0, sp: 0, mp: 0, stats: {str: 0, dex: 0, int: 0}},
};
...
export const genTarget = (level = 1) => {
    ...
    let hp = level * Math.max(1, 9 + target.stats.str + quality.hp + mod.hp + body.hp);
    target.hp = hp;
    target.max_hp = hp;

    let sp = level * Math.max(1, 4 + target.stats.dex + quality.sp + mod.sp + body.sp);
    target.sp = sp;
    target.max_sp = sp;

    let mp = level * Math.max(1, target.stats.int + quality.mp + mod.mp + body.mp);
    target.mp = mp;
    target.max_mp = mp;
