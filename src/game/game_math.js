...
export const attack = (state, params) => {
        ...
        state = attack(state,
            {
                ...
                onMiss: (state, Prob) => { state.chat.unshift({text: params.defender + " Counter Miss! Prob of hit " + Prob.toFixed(0) + '%'}); return state; },
            });
        return state;
    }
    ...
    if (_.random(0, 100) < Prob) {
        console.log();
        let bonus_stat = state[params.attacker].stats[attacker_weapon.bonus_stat];
        let atk = _.random(attacker_weapon.min_dmg + bonus_stat, attacker_weapon.max_dmg + bonus_stat);
