...
export const actions = {
            ...
            state = attack(state,
                {
                    ...
                    onMiss: (state, Prob) => { state.chat.unshift({text: attacker + "  Miss! Prob: " + Prob.toFixed(0) + '%'}); return state; },
                });
            return state;
        }},
            ...
            state = attack(state,
                {
                    ...
                    onHit: (state, dmg) => {
                        ...
                        return state; },
                    onMiss: (state, Prob) => { state.chat.unshift({text: attacker + " Push Miss! Prob: " + Prob.toFixed(0) + '%'}); return state; },
                });
            return state;
        }},
            ...
            state = attack(state,
                {
                    ...
                    onHit: (state, dmg) => {
                        ...
                        return state; },
                    onMiss: (state, Prob) => { state.chat.unshift({text: attacker + " Poison Miss! Prob: " + Prob.toFixed(0) + '%'}); return state; },
                });
            return state;
        }},
            ...
            state = attack(state,
                {
                    ...
                    onHit: (state, dmg) => {
                        ...
                        return state; },
                    onMiss: (state, Prob) => { state.chat.unshift({text: attacker + " Exhaust Miss! Prob: " + Prob.toFixed(0) + '%'}); return state; },
                });
            return state;
        }
    },
            ...
            state = attack(state,
                {
                    ...
                    onHit: (state, dmg) => {
                        ...
                        return state; },
                    onMiss: (state, Prob) => { state.chat.unshift({text: attacker + " Stun Miss! Prob: " + Prob.toFixed(0) + '%'}); return state; },
                });
            state.chat.unshift({text: attacker + "  name"});
            return state;
        }
    ...
    freeze: {name: "Freeze", cost: {'player.mp': 2},  
        ...
        onAction: (state, attacker, defender) => {
            ...
            state[attacker].action_timer += getActionDelay(30, state[attacker]);
            let atk = _.random(state[attacker].stats.wiz, state[attacker].level + state[attacker].stats.wiz);
    ...
    fire: {name: "Fire", cost: {'player.mp': 2},  
        ...
        onAction: (state, attacker, defender) => {
            ...
            state[attacker].action_timer += getActionDelay(30, state[attacker]);
            let atk = _.random(state[attacker].stats.int, state[attacker].level * state[attacker].stats.int);
    ...
    banish: {name: "Banish", cost: {'player.mp': 5},
        text: "Cuts half ot HP of defender, rounded down",
        isHidden: (state, attacker, defender) => state[attacker].stats.int < 5,
        isNotAllowed: (state, attacker, defender) => (state[attacker].action_timer),
        onAction: (state, attacker, defender) => {            
            state[attacker].action = 'banish';
            state[defender].hp = Math.ceil(state[defender].hp / 2);
            state.chat.unshift({text: defender + " Banished"});
            return state;
        }
    },
